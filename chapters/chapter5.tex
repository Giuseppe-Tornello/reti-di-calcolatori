...
Lo scopo di questi algoritmi è \textbf{determinare i migliori cammini} da usare tra le sorgenti e le destinazioni. Il miglior cammino è valutato rispetto a vari fattori, che riprendono ciò che abbiamo affrontato nello studio dei problemi di ottimizzazione sui grafi, nella materia Algoritmi e Laboratorio. \href{https://github.com/BoredDam/Algoritmi}{Vedi il mio GitHub per approfondimenti}.
...
\begin{itemize}
	...
	\item Negli \textbf{algoritmi di instradamento dinamici}, gli instradamenti variano in funzione di fattori quali il volume del traffico e la topologia della rete. L'algoritmo viene eseguito periodicamente o in funzione di variazioni nella topologia o di costi relativi ai collegamenti.
\end{itemize}
\newpage
...
Dati in input un grafo $G$, un nodo sorgente $s$ e una struttura dati contenente gli archi e il loro peso $w$, Dijkstra calcola tutti i cammini minimi che partono da $s$, verso ogni nodo $v \in G.V$.\\
...
\begin{enumerate}
	...
	\item Si inserisce $u$ nell'insieme $S$.
...
\subsubsection{Iterazione}
\begin{enumerate}
	\item Si aspetta fino a quando non ricevi un vettore delle distanze, o finché non cambia il costo verso un nodo adiacente.
	
	\item Ricevuto il vettore delle distanze da un nodo $D_y$, si andrà a calcolare, per ogni $v \in G.V$, 
	$$
	D_x[v] = \min \{D_x[v], \;D_y[v] + w(x,y)\}
	$$
	ovvero, se esiste, calcolo il cammino di peso inferiore per andare da $x$ a $v$, passando per $y$. Se questa condizione si verifica, e la $D_x[v]$ di qualsiasi nodo $v$ cambia, inoltra ll vettore delle distanze a tutti i nodi adiacenti.
	
	\item Ritorna allo step 1 dell'iterazione.
\end{enumerate}
Si tratta di un processo \textbf{asincrono}, che\textbf{ itera un numero indefinito di volte su ogni nodo}, in cui i valori minimi calcolati non cambiano, se non successivamente a cambiamenti della rete. In tal caso, ogni nodo il cui vettore delle distanze cambia, inoltrerà quest'ultimo agli adiacenti.
...
\begin{itemize}
	\item All'interno di un sistema autonomo, tutti i router eseguono lo stesso algoritmo di routing, chiamato \textbf{protocollo di instradamento intra-AS}.
...
\begin{itemize}
	...
	\item Un percorso non aggiornato per 180 secondi è marcato come irraggiungibile.
...
\begin{itemize}
	...
	Permettendo a ciascun sistema di rendersi visibile rispetto ai \textbf{prefissi di rete} che gestisce. È tramite BGP che tutti i router conoscono le sottoreti vicine.
