...
\subsection{Connessione logica, connessione reale}
...
\textbf{Il demultiplexing avviene al destinatario}. Consiste nella \textbf{consegna dei segmenti al socket corretto}, e quindi al processo corretto, sfruttando le specifiche degli header, tra cui, chiaramente, la porta.
...
Quello appena calcolato, è da definirsi un \textbf{throughput teorico}. Il \textbf{throughput effettivo} è influenzato anche da altri elementi di attesa, quali \textit{Round Trip Time}, tempi di elaborazione e di trasferimento relativi agli \textbf{ACK}.
...
$$
a cui aggiungere $30000 \mu s$. Otteniamo un throughput effettivo di
$$
...
Osserviamo che se l'$n$-esimo pacchetto non è stato ricevuto con successo, tutti i successivi pacchetti mandati verranno scartati. Nell'immagine della prossima pagina, si evidenzia come il primo inoltro dei pacchetti 3, 4 e 5 viene scartato. È un algoritmo solido e reliable, ma rischia grosse perdite di tempo a causa dell'\verb|ACK| cumulativo.
\newpage
...
\section{Migliorare il Throughput - MMS e MTU}
...
\section{Migliorare il Throughput - Retransmission Time-Out}
Affrontando l'argomento del \textbf{Reliable Data Transfer}, abbiamo spiegato che il meccanismo di \textbf{temporizzazione è fondamentale} per ottenere un trasferimento dati \textbf{reliable}. Nonostante ciò, bisogna trovare un buon \textbf{compromesso} per non impattare negativamente sul throughput. \textbf{Il time-out va stabilito in funzione del Round Trip Time}.
...
\end{enumerate}
\textbf{Cosa succede se l'ACK viene smarrito?}
...
\begin{itemize}
	...
	La rete non dà alcun tipo di feedback: la congestione è \textbf{dedotta} da ritardi dei pacchetti e ritardi.
\end{itemize}
...
\begin{enumerate}
	...
	Il mittente inizia con una congestion window di 1, ma che cresce, moltiplicata per 2 ad ogni RTT, fino a una soglia stabilita dalla variabile \verb|sshtresh|. Oltre quella soglia, si entra in congestion avoidance.
	
	\item \textbf{Congestion Avoidance}.\\
	Superata la \verb|sshtresh|, e quindi circa la soglia di $\frac{\text{cwnd}}{2}$, inizia la congestion avoidance: \textbf{la crescita diventa lineare} e \verb|sshtresh|= \verb|cwnd|/2.
	...
	\begin{enumerate}
		...
		Il valore di \verb|cwnd| incrementa di 3 MSS, e quindi \verb|sshtresh| = $(\verb|cwnd|+3)/2$.
	\end{enumerate}
...
\begin{itemize}
	...
	Se si perde un pacchetto (individuata da timeout o 3 ACK duplicati), la \verb|sshtresh| viene dimezzata, e il cwnd viene azzerato, ripartendo da 1 MSS a priori. Si rientra sempre in slow start dopo qualsiasi evento di perdita.
	\item \textbf{TCP-Reno.}\\
	Se si perde un pacchetto (solo se individuata 3 ACK duplicati), fast recovery! \verb|sshtresh| viene dimezzata, e il \verb|cwnd| riparte da un valore $\approx \verb|sshtresh|$.\\
