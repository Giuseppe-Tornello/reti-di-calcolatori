...
\begin{itemize}
	...
	\begin{itemize}
		...
		raggruppando simboli/bit del layer fisico in frame e \textbf{trasformando il flusso grezzo} di \textbf{bit} in informazioni strutturare. 
	...
	I dispositivi, nel DLL, sono identificati tramite indirizzi MAC, dei veri e propri indirizzi fisici. L'accesso al canale condiviso, nel caso dei \textbf{collegamenti broadcasat}, dev'essere gestito tramite dei \textbf{protocolli d'accesso multiplo}, per evitare \textbf{collisioni}.
	...
	\item \textbf{Regola il flusso dati} tra sorgente e destinazione.\\
...
Moltiplicando il numero il numero ottenuto, per il numero di step (o simboli) al secondo, otteniamo il \textbf{limite teorico di bitrate}.

$$
\text{Limite teorico bit-rate} = \text{Simboli al secondo} \cdot \log_2 n
$$
\subsubsection{Piccolo approfondimento sulla quanittà di informazioni - non è nel programma}
...
\begin{enumerate}
	...
	\item Effettuamo la divisione $\mod 2$ tra $\frac{D\cdot 2^r}{G}$.
...
\begin{enumerate}
	...
	\item $B$ trasmette. Collissione!
\end{enumerate}


\subsubsection{CSMA-CD - Carrier Sense Multiple Acces con Collision Detection}
...
\begin{itemize}
	...
	\item \textbf{Cat6}, che supporta fino a 10Gbps e opra sui 250Mhz di frequenza;
...
\begin{itemize}
	...
	È il tag presente \textbf{esclusivamente nelle frame Ethernet che supportano le VLAN}. Non è sempre presente, in quanto non tutti i frame Ethernet sono \textit{tagged}. 4 byte.
...
Ciò che viene fatta, è una \textbf{Trellis code modulation}. Raddoppiando il numero di bit per rappresentare un bit d'informazione, gli errori peseranno meno sull'informazione complessiva. Tuttavia è grazie alle transizioni dell'automa limitate che è possibile capire quando è avvenuto un errore: la distanza di Hamming delle stringhe possibili valutate rispetto a quella consegnata, ci permette di stabilire come effettuare la correzione.
...
\subsection{Switch}
È un dispositivo che trasmette pacchetti ricevuti a uno o più dispositivi destinatari. È più espandibile in termini di numero di porte, rispetto al bridge, e effettua operazioni di \textbf{inoltro} (individuare a quale interfaccia mandare il frame) e \textbf{filtraggio} (capire quando scartare un frame). Uno switch contiene al suo interno una \textbf{switch table} con record del tipo \verb|Indirizzo MAC - Interfaccia - Tempo|.
...
\begin{itemize}
	...
	1 bit che pecifica se il frame è untagged. Il flag a 0 indica che il meccanismo di VLAN non è supportato.
